...
Private Sub Worksheet_Change(ByVal Target As Range)
    ...
    ' "Permission set" is in column G
    Dim permissionSetColumn As Range
    Set permissionSetColumn = Range("G:G")
    
        ' "Permission set Group" is in column H
    Dim permissionSetGroupColumn As Range
    Set permissionSetGroupColumn = Range("H:H")
    ...
    Dim rng As Range
    Dim cell As Range
    Set rng = Intersect(Target, Me.Range("F:I")) ' Monitor changes in columns F to I
    If Not rng Is Nothing Then
        For Each cell In rng
            If cell.Column = 9 Then ' Action column
                Dim rowNumber As Long
                rowNumber = cell.Row
                
                If cell.Value = "ADD" Or cell.Value = "DELETE" Then
				    If Me.Cells(rowNumber, 6).Value <> "" Then
						If MsgBox("For ADD/DELETE Action, PS or PS Group or combination of both coulmns are Mandatory .Hence clearing Reference Email", vbYesNo + vbQuestion, "Alert") = vbYes Then
							' Clear and disable column F
							Me.Cells(rowNumber, 6).ClearContents
							Me.Cells(rowNumber, 6).Locked = True
							' Enable columns G and H
							Me.Cells(rowNumber, 7).Locked = False
                ...
                ElseIf cell.Value = "MIMIC" Then
				    If Me.Cells(rowNumber, 7).Value <> "" or  Me.Cells(rowNumber, 8).Value <> "" Then	
						If MsgBox("For MIMIC Action, Reference Email coulmn is Mandatory .Hence clearing PS and PSGroup", vbYesNo + vbQuestion, "Alert") = vbYes Then
							' Clear and disable columns  G, and H
                ...
                ' Lock columns F, G, H, and the Action column after initial change
                If cell.Value <> "ADD" Or cell.Value <> "DELETE" Then
                    Me.Cells(rowNumber, 6).Locked = True
                ElseIf cell.Value <> "MIMIC" Then
                    Me.Cells(rowNumber, 8).Locked = True
                ...
                Me.Cells(rowNumber, 9).Locked = True
                Exit For
            ElseIf cell.Column = 6 And Me.Cells(cell.Row, 9).Value = "ADD" Or Me.Cells(cell.Row, 9).Value = "DELETE" Then
                ' If column F is edited and Action is still ADD, clear and disable it
                Application.EnableEvents = False
                Me.Cells(cell.Row, 6).ClearContents
                Me.Cells(cell.Row, 6).Locked = True
                Application.EnableEvents = True
            ElseIf cell.Column = 7 And Me.Cells(cell.Row, 9).Value = "MIMIC" Then
                ' If column F is edited and Action is still ADD, clear and disable it
                Application.EnableEvents = False
                ...
                Application.EnableEvents = True
                Exit For
            ElseIf cell.Column = 8 And Me.Cells(cell.Row, 9).Value = "MIMIC" Then
                ' If column F is edited and Action is still ADD, clear and disable it
                Application.EnableEvents = False
